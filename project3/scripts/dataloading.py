...
def preprocess_text(text):
    """
    Return tokenized text, after some preprocessing.
    """
    text = text.lower()  # convert to lowercase
    text = re.sub(r"https?:\/\/t.co\/[A-Za-z0-9]+", "", text)  # remove urls
    text = contractions.fix(text)  # e.g. don't -> do not, you're -> you are
    text = re.sub(r"\b[a-z]\b", "", text)  # remove single character words

    tokenizer = RegexpTokenizer(pattern='\w+')  # tokenize and keep only alphanumerics
...
class SentenceDataset(Dataset):
    ...
    def __init__(self, X, y, word2idx):
        ...
        # EX2
        self.data = [preprocess_text(text) for text in X]
        self.labels = y
    ...
    def __getitem__(self, index):
        ...
        for i in range(min(len(datum), max_len)):
            word = datum[i]
            example[i] = self.word2idx[word] if word in self.word2idx else self.word2idx["<unk>"]

        label = self.labels[index]
        length = min(len(datum), max_len)

        return np.array(example, dtype='int64'), label, length
